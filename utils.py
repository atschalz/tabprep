...
def make_cv_scores_with_early_stopping(target_type, scorer, cv, early_stopping_rounds=20, vectorized=False):
        ...
        for train_idx, test_idx in cv.split(X_df, y_s):
            ...
            if isinstance(final_model, (lgb.LGBMClassifier, lgb.LGBMRegressor)):
                pipeline.fit(
                    ...
                    **{
                        "model__eval_set": [(X_te, y_te)],
                        "model__callbacks": [lgb.early_stopping(early_stopping_rounds, verbose=False)],
                        # "model__verbose": False
                    }
                )
            else:
