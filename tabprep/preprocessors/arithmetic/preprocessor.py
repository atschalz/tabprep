...
class ArithmeticPreprocessor(BasePreprocessor):
    ...
    def _fit(self, X_in, y_in = None):
        ...
        if self.reduce_memory_usage:
            with timelog.block("reduce_memory_usage_base"):
        ...
        with timelog.block("basic_filter_base"):
            X = basic_filter(X, use_polars=True, cat_as_num=self.cat_as_num, min_cardinality=self.min_cardinality) # TODO: Make data adaptive and use more restrictive threshold for large datasets
        ...
        with timelog.block("advanced_filter_base"):
        ...
        if self.use_target_corr:
            with timelog.block("target_corr_base"):
                target_corr = get_target_corr(X, y, top_k=5, use_polars=True)
            print("Top base feature correlations:")
            print(target_corr)

        # 4. Generate all bivariate interactions
        with timelog.block("get_bivariate"):
            X_int = get_all_bivariate_interactions(X, order=2, max_base_interactions=10000000)

        with timelog.block("reduce_memory_usage_bivariate"):
            X_int = reduce_memory_usage(X_int, verbose=True, rescale=False)
        print(f"Generated {X_int.shape[1]} bivariate interaction features")

        if self.use_target_corr:

            with timelog.block("target_corr_bivariate"):
                target_corr = get_target_corr(X_int, y, top_k=5, use_polars=True)
            print("Top bivariate interaction feature correlations:")
            print(target_corr)

        # 5. Filter bivariate interaction features 
        n_feats_start = X_int.shape[1]
        # basic
        with timelog.block("basic_filter_bivariate"):
            X_int = basic_filter(X_int, use_polars=True, cat_as_num=self.cat_as_num, min_cardinality=self.min_cardinality)
            print(f"Using {len(X_int.columns)}/{n_feats_start} features after basic filtering")

        # based on cross-correlation with base features
        n_feats_start = X_int.shape[1]
        if self.use_cross_corr:
            with timelog.block(f"cross_correlation_bivariate"):
                use_cols = filter_cross_correlation(X, X_int, corr_threshold=self.corr_threshold, block_size=self.cross_corr_n_block_size)
            X_int = X_int[use_cols]
        ...
        if X_int.shape[1] > self.max_accept_for_pairwise:
            print(f"Limiting interaction features to {self.max_accept_for_pairwise} (from {X_int.shape[1]})")
            X_int = X_int.sample(n=self.max_accept_for_pairwise, random_state=self.rng, axis=1)

        n_feats_start = X_int.shape[1]
        with timelog.block("int_spearman_bivariate"):
            use_cols = filter_spearman_pl_from_pandas(X_int, corr_threshold=self.corr_threshold, verbose=True)
            X_int = X_int[use_cols]
        print(f"Using {len(use_cols)}/{n_feats_start} features after feature interaction correlation filtering")

        if self.use_target_corr:
            target_corr = get_target_corr(X_int, y, top_k=5, use_polars=True)
            print("Top bivariate interaction feature correlations after filtering:")
            print(target_corr)

        self.new_feats.extend(use_cols)

        run_higher_order = True
        if len(self.new_feats) >= self.max_new_feats:
            print(f"Reached max new features limit of {self.max_new_feats}. Stopping.")
            run_higher_order = False
        ...
        if run_higher_order:
            cols_per_order = {2: X_int.columns.tolist()}
            for order in range(3, self.max_order+1):
                ...
                with timelog.block(f"get_interactions_{order}-order"):
                    X_int_higher = add_higher_interaction(X, X_int, max_base_interactions=10000)

                with timelog.block(f"reduce_memory_{order}-order"):
                ...
                if self.use_target_corr:
                    with timelog.block(f"target_corr_{order}-order"):
                ...
                # basic
                with timelog.block(f"basic_filter_{order}-order"):
                    X_int_higher = basic_filter(X_int_higher, use_polars=True, cat_as_num=self.cat_as_num, min_cardinality=self.min_cardinality)
                ...
                n_feats_start = X_int_higher.shape[1]
                with timelog.block(f"spearman_int_filter_{order}-order"):
                ...
                if self.use_cross_corr:
                    n_feats_start = X_int_higher.shape[1]
                    with timelog.block(f"cross_correlation_{order}-order"):
                ...
                cols_per_order[order] = X_int_higher.columns.tolist()
        ...
        # self.new_feats_compiled = [e.replace('_/_', '/').replace('_x_', '*').replace('_+_', '+').replace('_-_', '-') for e in self.new_feats]
        self._compile_expressions()
        self.time_logs = timelog.summary()
        return self
