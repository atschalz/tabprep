...
class GroupByFeatureEngineer(BasePreprocessor):
    ...
    def groupby(self, X_num_in, X_cat_in, seed=42, **kwargs):
        ...
        for col1, col2 in zip(feat_combs_use_arr[0], feat_combs_use_arr[1]):
            ...
            # self.rank_map[col1 + "_by_" + col2+'_RANK'] = X_num[col1].groupby(X_cat[col2], observed=False).apply(lambda x: dict(x.rank()/x.sum()))
            # new_col = X_cat[col2].map(self.rank_map[col1 + "_by_" + col2+'_RANK']).astype(float).fillna(0)
            # # new_col = new_col - X_num[col1]
    ...
    def fit(self, X_in, y_in=None):
        ...
        X_cat = X.loc[:,(X.nunique()>1).values].select_dtypes(include=['category', 'object'])#.astype('U') #TODO: Check whether astype assignment at this point is best
        X_num = X.loc[:,(X.nunique()>2).values].select_dtypes(exclude=['category', 'object'])
    ...
    def transform(self, X_in):        
        ...
        if len(self.new_col_set) > 0:
            X_cat = X_out.loc[:,(X_out.nunique()>1).values].select_dtypes(include=['category', 'object'])#.astype('U') #TODO: Check whether astype assignment at this point is best
            X_num = X_out.loc[:,(X_out.nunique()>2).values].select_dtypes(exclude=['category', 'object'])
            ...
            # X_new_rank = self.groupby_predefined(X_num, X_cat, [i for i in self.new_col_set if '_RANK' in i], mode='RANK')
